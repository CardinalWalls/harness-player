...
if ! python3 "$ROOT/scripts/mcp_http_call.py" --url "$URL" initialize --params '{"protocolVersion":"2025-06-18","capabilities":{},"clientInfo":{"name":"clone-hermes-status","version":"0.1.0"}}' >/dev/null 2>&1; then
  echo "endpoint: $URL (unreachable)"
  exit 1
fi

echo "endpoint: $URL (ready)"
python3 "$ROOT/scripts/mcp_http_call.py" --url "$URL" tools/call --params '{"name":"clone_status","arguments":{}}'
