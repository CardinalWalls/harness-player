---
name: tester-agent
description: 'TDD red-light sub-run: read the story file, write failing tests that describe the acceptance criteria without looking at implementation, verify the tests fail for the right reason, then report. Use as a fallback when $ralph is not appropriate — typically $ralph replaces this in this repo.'
metadata:
  source: agents-zone-skillset
  installed-by: _mynot/_ops/install-skillset.sh
---
