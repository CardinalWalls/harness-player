...
<workspace_layering>
## Workspace layering (authoritative pointer)

This repo enforces a five-layer document discipline for any SSOT / architecture / plan / runbook work. The authoritative contract lives at **`_mynot/AGENTS.md`**; read it before producing or editing any of:

- `_mynot/0-bigger-project-context.md` (Layer 0 — big-project context)
- `_mynot/1-intent/PRD.md` (Layer 1 — intent, produced by `/prd`)
- `_mynot/2-architecture/ARCHITECTURE.md` (Layer 2 — architecture, produced by `/architect`)
- `_mynot/3-plan/stories/*.md` (Layer 3 — per-story plan, produced by `/story`; single story then handed to `$ralplan`)
- `_mynot/4-runbook/RUNBOOK-*.md` (Layer 4 — runbook, produced after Layer 5 stabilizes)
- any impl code that claims to realize the above (Layer 5)

### OMX workflow gating by layer

`_mynot/AGENTS.md §4` specifies where each OMX workflow is allowed:

- `$deep-interview` — Layer 1 only, before `/prd`, to surface "don't assume" gaps.
- `$ralplan` — Layer 3 only, **scoped to a single story** already frozen in `_mynot/3-plan/stories/`. Forbidden to run against `_mynot/1.md`, Layer 0, or whole-PRD input.
- `$ralph` — Layer 5, only after `$ralplan` produced `.omx/plans/prd-<story>.md` + `test-spec-<story>.md` for that story.
- `$team` / `$swarm` — Layer 5, only when multiple stories are independent and each has its own `.omx/plans/*` pair.

### Cross-session state

Repository-root `PROGRESS.md` is the single source of current progress and the state file read by `/conduct` (agents-zone-skillset). Any agent starting a session reads it first; any agent ending a session updates it. See `_mynot/AGENTS.md §6`. (The earlier path `_mynot/PROGRESS.md` is deprecated — do not create or write that file.)

### Lesson routing (prevents principles from getting lost again)

When a principle stated earlier turns out to be missing from current artifacts, route it back via `/hr <lesson>` and update the upstream skill/agent doc — do not patch only the downstream artifact. See `_mynot/AGENTS.md §7`.

### Precedence

When this section and `_mynot/AGENTS.md` disagree, `_mynot/AGENTS.md` wins for layered workspace work. Otherwise the rest of this file (root `AGENTS.md`) governs how the agent operates in general (delegation, models, git, skills, keywords).
</workspace_layering>
...
## Intent Freeze & Mock-First Contract
- For any architecture change, product-direction change, workflow change, demo semantics change, or UI/storytelling change, implementation is forbidden until intent is frozen first.
- The required sequence is:
  1. create a versioned **intent freeze** artifact
  2. create a versioned **mock / expected result** artifact
  3. obtain explicit user confirmation on that mock/result
  4. only then implement
- For the `_mynot/`-layered workflow specifically, step 3 (explicit user confirmation) is satisfied mechanically by passing the `_mynot/AGENTS.md §3` freeze checklists; human approval is summoned only via `STORY-BLOCKED: <id>` appended to `PROGRESS.md` (see `_mynot/_ops/skills/conduct.md`). The layered workspace encodes the same acceptance intent as machine-checkable rules, so the human step is reserved for the BLOCKED case.
- The intent freeze and mock artifacts must live in the repo and be tracked by version control.
- The default versioned context tool for this repo is **Git + Entire**. If Entire is available, keep it enabled and use commit-backed checkpoints for intent freeze, mock freeze, and implementation milestones.
- When the repo is not yet under Git, initialize Git before starting architecture/product implementation work. Enable Entire when feasible.
- Do not treat derived summaries, inferred flows, or capture-parsed views as native contracts. Label them honestly as captured/parsed/derived until a real contract exists.
- If the user rejects a spec, mock, or implementation as misleading/deceptive/wrong-intent, do **not** refine that implementation in place by default. Revert/remove the rejected work, return to spec, and rebuild from an approved frozen intent.
- If the user provides corrective feedback on a wrong spec, the next deliverable is an optimized replacement spec/mock unless the user explicitly asks to implement immediately.
- Future commits for this workflow should prefer three lanes when applicable:
  - `intent freeze`
  - `mock freeze`
  - `implementation`
